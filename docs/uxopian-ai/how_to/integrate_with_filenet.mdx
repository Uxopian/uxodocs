...
NOT match `/gui/gateway/uxopian-ai/filenet/...` — path-prefix matching is path-element aware, so
a rule for `/gui/gateway/uxopian-ai` does not cover `/gui/gateway/uxopian-ai-filenet` either, if
you're still on an older gateway version using that non-fixed path. Verify with a plain
`curl -I <host>/gui/gateway/auth/login` (expect a gateway response, not a 404 from some other
backend) before assuming the ICN plugin's admin/chat flows will work end-to-end.
:::
...
| `AI Admin` opens a 404 at `.../gui/gateway/auth/login?providerId=FileNetProvider&token=...` (or the chat/prompt calls 404 while other gateway routes on the same host work fine) | The gateway sits behind an ingress whose rule for `/gui/gateway/uxopian-ai` doesn't cover `/gui/gateway/uxopian-ai/filenet` as a sub-path — a broader catch-all rule (e.g. a bare `/gui`) is intercepting the request first and routing it to the wrong backend entirely | Confirm the ingress rule for `/gui/gateway/uxopian-ai` uses a prefix `pathType` broad enough to include `/filenet` sub-paths and points at the gateway service; test with `curl -I <host>/gui/gateway/auth/login` |
