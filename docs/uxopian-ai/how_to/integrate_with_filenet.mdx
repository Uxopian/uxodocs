...
| FileNet Content Engine | **5.5.9 or later** — required for `OpenTokenCredentials` (bearer-token authentication), used for per-caller impersonation — see [Phase 0.4](#04-configure-oidc-trust-on-the-content-platform-engine-acce) |
...
| ACCE access to the P8 domain | Required to register the OIDC trust that lets the Content Platform Engine authenticate callers as themselves — see [Phase 0.4](#04-configure-oidc-trust-on-the-content-platform-engine-acce) |
...
Every FileNet call is authenticated as the **current caller**, not a shared service account. For each request, `FileNetCeClient` reads the authenticated user (id + token) from `AiContext` — populated from the JWT the gateway forwards as `X-User-Token` — and wraps them in an `OpenTokenCredentials(userId, token, oidcRealm)`, then runs the Content Engine call via `Credentials.doAs(...)`. FileNet therefore logs and authorizes every operation against the real end user. `oidc-realm` is the name of the OIDC/OAuth provider trust registered on the Content Platform Engine that lets it accept this token — see [Phase 0.4](#04-configure-oidc-trust-on-the-content-platform-engine-acce). It must match that realm name exactly, and there is no fallback: if the caller has no identity or token in `AiContext`, the call fails rather than silently using a shared account.
:::

### 0.4 Configure OIDC trust on the Content Platform Engine (ACCE)

Per-caller impersonation only works if the Content Platform Engine trusts the JWT issued by the ICN plugin. This trust is registered once per environment, directly on the P8 domain — it is infrastructure configuration, not something set in `uxopian-ai` or the ICN plugin:

1. Open ACCE (Administration Console for Content Platform Engine) and connect to the P8 domain hosting the target object store.
2. Register an OIDC/OAuth identity provider trust (the exact menu path depends on your CE release — consult IBM's FileNet P8 documentation for your version) using:
   - the same RSA **public key** exposed by the ICN plugin at `?op=public-key` ([Phase 3.1](#31-register-the-plugins-public-key-with-filenetprovider))
   - the same **issuer** as the plugin's JWTs (the `icn.plugin.jwt.issuer` JVM property, default `icn-plugin`)
3. Name the provider/realm — this is the value to set in `filenet.oidc-realm` / `FILENET_OIDC_REALM`.
4. Confirm that the identities carried by the JWT (the ICN user id) resolve to valid users in the P8 domain's directory — CE authorizes and audits calls against this identity directly, so it must already exist there.

:::warning[Re-register whenever the plugin's keystore changes]
If the ICN plugin's keystore is replaced ([Modifying the plugin](#replacing-the-keystore)), the public key changes. Update both the gateway's `filenet.jwt.public-key` ([Phase 3.1](#31-register-the-plugins-public-key-with-filenetprovider)) and the OIDC trust registered in ACCE, or impersonation will start failing with `E_NOT_AUTHENTICATED`.
:::
...
| `filenet.oidc-realm` | `FILENET_OIDC_REALM` | (empty) | Name of the OIDC/OAuth provider trust registered on the Content Platform Engine via ACCE ([Phase 0.4](#04-configure-oidc-trust-on-the-content-platform-engine-acce)). Used to build a per-caller `OpenTokenCredentials` — every CE call is authenticated as the current end user, not a shared account. Required. |
...
| `E_NOT_AUTHENTICATED` from FileNet on every call | The Content Platform Engine does not trust the caller's JWT, or `filenet.oidc-realm` does not match the realm registered in ACCE | Confirm the OIDC provider trust in ACCE uses the same public key and issuer as the ICN plugin, and that `filenet.oidc-realm` matches its realm name exactly (Phase 0.4) — also check CE is 5.5.9+ |
