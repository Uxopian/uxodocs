...
request too, injecting `FlowerDocsProvider` instead of `FileNetProvider`. Symptoms are confusing
because most of the flow still appears to work: the login exchange succeeds and a session cookie
is set, but it's stored keyed by `FileNetProvider`; the very next request then fails to resolve
...
:::warning[If the gateway sits behind an ingress, `/gui/gateway/uxopian-ai/filenet` must be routed there too]
This path is a sub-path of `/gui/gateway/uxopian-ai`, so most ingresses already routing that
prefix (for the default FlowerDocs route) cover it automatically. But an ingress with a rule
matching `/gui/gateway/uxopian-ai` **exactly** (not as a shared prefix covering sub-paths) will
NOT match `/gui/gateway/uxopian-ai/filenet/...`, and `/gui/gateway/auth/login` (the login-exchange
endpoint the **AI Admin** button needs) is a *sibling* of `/gui/gateway/uxopian-ai`, not a child
of it — so it needs its own ingress coverage regardless of how the FileNet route itself is
configured. The safest single ingress rule is a prefix on `/gui/gateway` (not
`/gui/gateway/uxopian-ai`), covering both. Verify with a plain `curl -I <host>/gui/gateway/auth/login`
(expect a gateway response, not a 404 from some other backend) before assuming the ICN plugin's
...
- The `buildChatRequest` function — builds the initial chat payload (`{ inputs: [] }`, no predefined system prompt; add one here if your deployment wants the chat to open with a fixed assistant persona — see [Common issues](#common-issues) for what's required if you do)
- The `openAdmin` function (invoked by the **AI Admin** toolbar button) — resolves the host via `fetchUxopianHost()` (rejecting with an explicit error if unconfigured), fetches an `admin-token`, then opens `GET <login-url>?providerId=FileNetProvider&token=…&redirect=<admin-path>` in a new tab so the gateway sets the `GATEWAY_SESSION` cookie in a first-party context before redirecting to the admin panel. `<login-url>` is derived by anchoring on the gateway's fixed `/gui/gateway/` base and replacing everything after it with `auth/login` (matching `app.gateway.login-path`, default `/gui/gateway/auth/login`) — **not** by trimming a fixed number of trailing segments off the configured host, since that count varies (compare `.../uxopian-ai/filenet` here to a different provider's route depth).
...
| `AI Admin` opens a 404 at `.../gui/gateway/auth/login?providerId=FileNetProvider&token=...` (or the chat/prompt calls 404 while other gateway routes on the same host work fine) | The gateway sits behind an ingress whose rule for `/gui/gateway/uxopian-ai` doesn't cover `/gui/gateway/auth/login` (a sibling path, not a sub-path) or `/gui/gateway/uxopian-ai/filenet` — a broader catch-all rule (e.g. a bare `/gui`) is intercepting the request first and routing it to the wrong backend entirely | Use a single ingress rule on the broader `/gui/gateway` prefix rather than `/gui/gateway/uxopian-ai` specifically (Phase 3.2); test with `curl -I <host>/gui/gateway/auth/login` |
| `AI Admin` opens the correct URL and the redirect chain completes, but the final admin page 401s with `Authentication token is missing` in the gateway's logs, even though a `GATEWAY_SESSION` cookie was set | `uxopian-ai-filenet` is declared **after** `uxopian-ai` in the routes list — the broader `/gui/gateway/uxopian-ai/**` pattern shadows it, so the request gets `FlowerDocsProvider` injected instead of `FileNetProvider`, and the session (stored under the `FileNetProvider` key) can't be resolved under the wrong provider | Move `uxopian-ai-filenet` (and its `-ws` counterpart) before `uxopian-ai` in the routes list (Phase 3.2) |
...
| Chat fails immediately with `System prompt with id <name> not found`, even for a plain message | `buildChatRequest()` in `actionHandler.js` was customized to reference a system prompt (`type: "PROMPT"`) that doesn't exist for this tenant yet — by default the plugin sends no system prompt (`{ inputs: [] }`) | Either remove the `PROMPT` entry from `buildChatRequest()` (see [Customizing the frontend behavior](#customizing-the-frontend-behavior)), or create a prompt with that exact id via the Admin UI / `POST /api/v1/admin/prompts` for this tenant |
