...
sidebar_position: 8
content_hash: 82d273848cdaaa6e7a10f1504c8309a885ef4826545a2f184d4916ad38d7a8ff
---
...
The CPE fetches the signing key dynamically, so rotating the plugin's keystore only requires updating the gateway's `filenet.jwt.public-key` ([Phase 3.1](#31-register-the-plugins-public-key-with-filenetprovider)) — nothing to redo on the CPE.
:::
...
:::danger[This source predates dynamic tenant resolution — do not deploy as-is on ft5]
`JwtSigner` in this archive signs `sub`/`iss`/`iat`/`exp`/`roles` only — **no `tenantId` claim** — and `UxopianProxyService` never calls anything resolving a repository/object store. Deployed as-is against the current gateway/uxopian-ai (which expect the JWT to carry `tenantId`, see [How the tenant is resolved](#how-the-tenant-is-resolved)), every request falls back to the gateway's static `filenet.jwt.tenant-id`, silently merging every object store the desktop exposes into one tenant instead of isolating them. The plugin needs its per-caller impersonation and tenant-resolution logic (documented above) actually implemented before this archive matches the guide. Do not build and deploy this JAR into a multi-object-store environment expecting isolation until it's updated.
:::
...
If you replace the plugin's keystore ([Replacing the keystore](#replacing-the-keystore)), the public key changes too. Re-fetch it from `?op=public-key` and update `filenet.jwt.public-key` on the gateway, then restart the gateway.
:::

:::tip[Serving more than one P8 domain from one gateway]
The single top-level `filenet:` block above supports exactly one `FileNetProvider` instance. Since 2026.0.0-ft5, the gateway can instead run **several named instances** — e.g. one per P8 domain/CPE — each with its own `public-key`/`issuer`/`tenant-id`, selected per route via `provider: <name>`. See [Configuration reference — Named provider instances](../reference/configuration.md#named-provider-instances-appproviders) (the config keys are the same, without the `jwt.` segment: `public-key`, `issuer`, `tenant-id`, `clock-skew-seconds`).
:::
...
| `filenet.jwt.tenant-id` | `filenet` | Fallback tenant, used only for tokens with no `tenantId` claim. Normally the object store from the claim wins — see [How the tenant is resolved](#how-the-tenant-is-resolved). |
