...
- **`UXO_AI_ENDPOINT` still going through Zuul.** If `UXO_AI_ENDPOINT` resolves to a path that FlowerDocs GUI intercepts, streaming is blocked (see [504](#504--proxy-timeout) above). Confirm that the Traefik priority 20 route for `/gui/gateway/**` is active.
