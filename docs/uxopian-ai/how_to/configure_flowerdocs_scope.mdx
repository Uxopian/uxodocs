...
:::warning[This archive needs a refresh]
This ZIP predates the current script source and still contains the old, non-scoped `GATEWAY_ENDPOINT` endpoint (the exact bug described in [Why WebSocket needs a separate, direct endpoint](#why-websocket-needs-a-separate-direct-endpoint) — it resolves the WebSocket to a route that no longer exists, `/ws/undefined`) and a `qp-connector` route matcher that assumes FlowerDocs component ids are always UUIDs (they aren't). The authoritative, current source is the `uxoai-flowerdocs` package (managed via `uxc`, kind `fd.script`) — re-export this archive from there via CLM before relying on it, or install directly from that package instead of this download.
:::
...
// Rendered from the wsGatewayUrl package variable; empty if not configured.
const WS_GATEWAY_URL_RAW = '{{uxc:wsGatewayUrl}}';
const WS_UXO_AI_ENDPOINT = WS_GATEWAY_URL_RAW.startsWith('{{')
  ? ''
