...
This quickstart connects to uxopian-ai running with `DevProvider` (authentication disabled). Before going to production, switch the gateway to `FlowerDocsProvider` so FlowerDocs session tokens are validated. See [Integrate with FlowerDocs](../how_to/integrate_with_flowerdocs.mdx) for the production-ready setup.
:::
