...
- A BFF or reverse proxy that injects the `X-Provider-ID: alfresco` header
...
services:
  ...
  service-broker:
    ...
    environment:
      ...
      - "CONNECTOR_REGISTRIES_ALFRESCO_WHITELISTED_PARAMS=nodeRef,versionLabel,docs,folder"
      - "CONNECTOR_DEFAULT_REGISTRY=alfresco"
...
The following query parameters are used by the provider. The broker forwards all incoming request parameters to the provider regardless of whether they appear in `CONNECTOR_REGISTRIES_ALFRESCO_WHITELISTED_PARAMS`.
