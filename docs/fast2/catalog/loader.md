...
| Key                            | Type          | Description                                                                                                                                                                                                                                                              | Default value |
| ------------------------------ | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------- |
| Write all in the same CSV file | `Boolean`     | Merge metadata of all punnets in a single output CSV file. The missing columns headers will be added on the fly. If set to false, any existing CSV file with the same name will be overwritten.                                                                          | `true `       |
| Whitelisted columns            | `String list` | Only these column names will be written to the output CSV. Column names must match the discovered metadata keys (e.g. 'punnetId', 'documentId', 'punnet.fileName', 'ObjectTypeName', 'doc.ObjectTypeName', 'folder.id'). If set, the blacklist is ignored. If both whitelist and blacklist are empty, all discovered columns are written. |               |
| Blacklisted columns            | `String list` | These column names will be excluded from the output CSV. Column names must match the discovered metadata keys (e.g. 'punnetId', 'documentId', 'punnet.fileName', 'ObjectTypeName', 'doc.ObjectTypeName', 'folder.id'). Only considered when the whitelist is empty. If both whitelist and blacklist are empty, all discovered columns are written. |               |
...
| Max number of records per csv  | `Integer`     | Define the maximum number of records per csv file, if the value is set to 0 all records will be in the same file                                                                                                                                                         | `0 `          |
| Add Folder Metadata (beta)     | `Boolean`     | For each document add its folder metadata on the same CSV line. Only the first level of folders will be stored into the CSV file. If a document is attached to more than one folder, its line will be cloned in the CSV file, to display one folder per line.            | `false `      |
