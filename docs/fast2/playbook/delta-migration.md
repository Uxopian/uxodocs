...
The per-punnet manifest is ephemeral, though, and a three-way match across 80M documents is not an exercise you want to run over a pile of batch files. This is where a [migration ledger](./migration-ledger.md) earns its place: carry `source_hash`, `source_bytes`, and `target_id` on the ledger row and the manifest stops being a separate artifact to collate. The match becomes a join. Note the limit, and it matters at audit time: the ledger is a first-party record of what Fast2 believes it did, so the third side of the match still has to be an independent enumeration of the target.
