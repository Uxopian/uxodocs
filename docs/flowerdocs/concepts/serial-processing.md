...
| ------- | --------- |
| Document search | ✅ |
| Virtual folder (bannette) | ✅ |
| Physical folder | ✅ |
| Saved search (from the search menu) | ✅ |
| Dashlets | ❌ |
...
:::note
If the first task opened is assigned to another user, serial processing is not available. The interface should indicate that the task is being processed by another user.
:::
