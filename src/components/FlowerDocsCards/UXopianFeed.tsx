            ...
            .then((data) => {
                ...
                const articleEls = Array.from(
                    doc.querySelectorAll(
                        'article, .post, [class*="post-card"], [class*="blog-card"], .blog-index_post'
                    )
                ) as HTMLElement[];

                articleEls.forEach((el) => {
                    const linkEl = el.querySelector(
                        'a[href*="/blog/"], a.blog-index_post-link'
                    ) as HTMLAnchorElement | null;
                    const titleEl = el.querySelector(
                        'h1, h2, h3, h4, .entry-title, [class*="title"]'
                    );

                    let imgEl = el.querySelector("img") as HTMLImageElement | null;
                    if (!imgEl) {
                        imgEl = el.querySelector(
                            '.blog-index_post-image img, [class*="image"] img'
                        ) as HTMLImageElement | null;
                    }

                    const excerptEl = el.querySelector(
                        'p:not([class*="meta"]), .excerpt, [class*="description"], [class*="content"] p'
                    );
                    ...
                    if (linkEl && titleEl) {
                        ...
                        if (title.length > 10 && !seenTitles.has(title)) {
                            ...
                            if (!imgUrl) {
                                const bgImgEl = el.querySelector(
                            ...
                            const link = linkEl.href.startsWith("http")
                                ? linkEl.href
                                : `https://www.uxopian.com${linkEl.href}`;

                            items.push({
                                title,
                                link,
                                excerpt: excerptEl
                                    ? (excerptEl.textContent || "").trim().substring(0, 160)
                                    : undefined,
                                image: imgUrl,
                                category: el
                                    .querySelector('[class*="category"], [class*="tag"]')
                                    ?.textContent?.trim(),
                                author: el
                                    .querySelector('[class*="author"], .blog-index_post-author')
                                    ?.textContent?.trim(),
                                date: el
                                    .querySelector('time, [class*="date"], .blog-index_post-date')
                                    ?.textContent?.trim(),
                            });
                        }
                    }
                        ...
                        <div className={styles.feedContent}>
                            ...
                            {(a.author || a.date) && (
                                <div className={styles.feedMeta}>
                                    {a.author && (
                                        <span className={styles.feedAuthor}>{a.author}</span>
                                    )}
