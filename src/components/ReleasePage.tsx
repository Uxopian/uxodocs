...
const ReleaseNoteCard = memo(function ReleaseNoteCard({
    note,
    styles,
    cardColor,
    readMoreLink,
...
export default function ReleasePage({
    ...
    const filterKeys: string[] = useMemo(() => {
        ...
        if (filterBy === "major") {
            return Array.from(new Set(allNotes.map((n) => n.majorVersion))).sort(
                        ...
                        <div className={styles.releaseGrid} key={selectedFilter}>
                            {enhancedNotes.map((note: any, index: number) => {
                                ...
                                return (
                                    <div
                                        key={note.version}
                                        style={{ animationDelay: `${animationIndex * 0.02}s` }}
                                    >
                                        <ReleaseNoteCard
                                            note={note}
                                            styles={styles}
