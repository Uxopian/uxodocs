...
> **Release note:** See [v2023.6.1](./release-notes).
...
- **_BEST_TARGET_**: Only select the Rendition server with the highest weather score
- **_WEIGHTED_DISTRIBUTION_**: Select the Rendition server based on a weighted random selection based on the weather score
- **_ROUND_ROBIN_** (new): Use a Round-Robin algorithm to iterate over the list of available rendition servers.
- **_RANDOM_** (new): Randomly select available server, regardless of the weather score.
...
- **_com.uxopian.arender.session.jvm.route_** indicates the unique server name to use to forge the session cookie.
- **_com.uxopian.arender.session.jvm.route.separator_** is the character separator between the session Id and the server name.
- **_com.uxopian.arender.session.jvm.base64 enables_** disables the final encoding of the cookie in Base64. This shall be disabled when the cookie value must be interpreted by IHS.
...
This setting can be changed using the option **_disk.free.space.threshold_** in the Rendition Broker configuration.
