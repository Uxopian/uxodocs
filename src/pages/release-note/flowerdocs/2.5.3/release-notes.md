...
✅ Les utilisateurs sélectionnent le scope sur lequel se connecter grâce à une liste déroulante. Cette liste propose les différents scopes auxquels s'est connecté l'utilisateur. _Ce nouveau champ peut être masqué en ajoutant le paramètre `scope.edit=false` dans le fichier `gui.properties`_
...
✅ Afin de faciliter le développement de scripts JavaScript, certaines classes peuvent être utilisées sans leur package _(exemples: `Document`, `SearchRequestBuilder`...)_
